...
* def adalah kata kunci yang digunakan untuk mendefinisikan atau membuat fungsi buatan sendiri untuk dipanggil lagi.
...
##### * Fungsi sebagai pemberi nilai: David menjelaskan bahwa alih-alih hanya menjalankan aksi seperti print() (yang merupakan side effect), sebuah fungsi bisa "mengembalikan" (return) sebuah nilai. return adalah instruksi di dalam sebuah fungsi yang bertugas untuk mengembalikan nilai hasil pemrosesan ke tempat fungsi tersebut dipanggil.Berbeda dengan print() yang sekadar menampilkan output ke layar secara visual lalu datanya hilang (None), return bekerja di balik layar untuk menyetorkan data nyata ke dalam sistem memori program. Dengan menggunakan return, hasil eksekusi dari sebuah fungsi dapat ditangkap, disimpan ke dalam variabel, atau diproses kembali untuk operasi kode selanjutnya.

* Konsep Return: Nilai yang dikembalikan oleh sebuah fungsi dapat disimpan ke dalam variabel atau digunakan kembali untuk perhitungan lebih lanjut. Ini membuat fungsi menjadi jauh lebih fleksibel daripada sekadar menampilkan sesuatu ke layar.
* Contoh: Dia menunjukkan bagaimana fungsi dapat melakukan perhitungan (seperti pemangkatan) dan mengirim hasilnya kembali ke pemanggil fungsi tersebut.
* Penyelesaian masalah: Di akhir segmen, ia mendemonstrasikan bahwa ada banyak cara untuk menyelesaikan masalah yang sama, misalnya menggunakan operator ** untuk pemangkatan atau menggunakan fungsi bawaan pow().
